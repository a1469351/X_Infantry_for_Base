...
{
	judge_process_data(judge_data);
	HAL_UART_Receive_IT(&CTRL_UART, &judge_data, 1);
}
...
void judgeUartInit(){
	...
	//zyk ????1??
	if(HAL_UART_Receive_IT(&JUDGE_UART, &judge_data, 1) != HAL_OK){
			Error_Handler();
	} 
}
...
void refreshdata()
{
  ...
  fw_printf("remainTime: %d \r\n",mytGameInfo.remainTime);
	
  mytGameInfo.remainLifeValue = (0x0000 | buffer[11]) | (buffer[12]<<8);
  fw_printf("remainLife: %d \r\n",mytGameInfo.remainLifeValue);
  ...
  fw_printf("COutA: %f \r\n",mytGameInfo.realChassisOutA);
	...
	mytGameInfo.locData.flag = buffer[21];
  fw_printf("POS OK: %d \r\n",mytGameInfo.locData.flag);
	
	b = (unsigned char*)&mytGameInfo.locData.x;
  c[0] = buffer[22];c[1] = buffer[23];c[2] = buffer[24];c[3] = buffer[25];
  for(int i = 0; i<4; i++){
      b[i] = (unsigned char)c[i];
  }
  fw_printf("POSx: %f \r\n",mytGameInfo.locData.x);
	
	b = (unsigned char*)&mytGameInfo.locData.y;
  c[0] = buffer[26];c[1] = buffer[27];c[2] = buffer[28];c[3] = buffer[29];
  for(int i = 0; i<4; i++){
      b[i] = (unsigned char)c[i];
  }
	fw_printf("POSy: %f \r\n",mytGameInfo.locData.y);
	
	b = (unsigned char*)&mytGameInfo.locData.z;
  c[0] = buffer[30];c[1] = buffer[31];c[2] = buffer[32];c[3] = buffer[33];
  for(int i = 0; i<4; i++){
      b[i] = (unsigned char)c[i];
  }
	fw_printf("POSz: %f \r\n",mytGameInfo.locData.z);
	
	b = (unsigned char*)&mytGameInfo.locData.compass;
  c[0] = buffer[34];c[1] = buffer[35];c[2] = buffer[36];c[3] = buffer[37];
  for(int i = 0; i<4; i++){
      b[i] = (unsigned char)c[i];
  }
	fw_printf("compass: %f \r\n",mytGameInfo.locData.compass);
	...
	if(receiving) 
		{
			...
			if(buffercnt == 44)
			{
				if (myVerify_CRC16_Check_Sum(buffer, 44)) refreshdata();
				receiving = 0;
				buffercnt = 0;
			}
		}
